...
import { appendQueryString, buildFormData, buildQueryString, lazyErr, merge, mergeHeaders, WithOnce } from './utils'
...
export function makeReq(config: AHttpConfig): ARequest {
  ...
  return {
    [Req]: true,
    url: new URL(config.url ?? location.href, config.base),
...
export function flowMergeReq(req: ARequest, config: AFlowMergeConfig): ARequest {
  ...
  if (config.url) req.url = new URL(config.url)
...
export function mergeReq(req: ARequest, obj: ARequest): ARequest {
  if (!obj) return req
  if (obj.url) req.url = new URL(obj.url)
...
export function mergeRes<T>(res: AResponse<T>, obj: AResponse<T>): AResponse<T> {
  ...
  if (obj.url) res.url = new URL(obj.url)
  return res
}
